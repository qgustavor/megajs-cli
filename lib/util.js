...
function downloadFileTo (file, target, options = {}, callback = defaultDownloadCallback) {
    ...
    if (!err) {
      if (options.continue && stats.size < file.size) {
        ...
        if (!options.verify) {
          ...
          verifyFile(target, file, options, error => {
  ...
  function ensureCallback (err) {
    ...
    downloadStream
      .on('end', () => { {
        if (startByte && options.verify && target !== '-') {
          verifyFile(target, file, options, error => {
            if (error) return callback(error)
            callback(null)
          })
...
function verifyFile (path, file, options, cb) {
  const verifyStream = mega.verify(file.key)
  let readStream = fs.createReadStream(path)
  if (options.progress) {
    const progressStream = createFileProgressStream('Integrity check', file.size, 0)
    readStream = readStream.pipe(progressStream)
  }
  readStream.on('error', cb)
  verifyStream.on('error', cb)
  verifyStream.on('end', () => cb(null))
  ...
  downloadFileTo(item.file, path.join(target, item.filePath), options, (err) => {
    if (err && err.message === 'File exists') {
      if (!options.disableExistsMessage) {
        if (options.verify) {
          console.error('%s exists and is valid', item.filePath)
        } else {
          console.error('%s exists', item.filePath)
        }
      }
      downloadFolderLoop(target, options, queue, cb)
      return
