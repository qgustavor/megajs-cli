...
function applyProxySettings (api, proxy) {
  if (proxy) {
    api.requestModule = api.requestModule.defaults({ proxy })
  }
}

function downloadFolder (folder, target, options, cb = () => {}) {
  const queue = createFileList(folder, '')
...
function downloadFileTo (file, target, options = {}, callback = () => {}) {
  ...
  fs.stat(target, (err) => {
    ...
    fs.ensureDir(path.dirname(target), (err) => {
      ...
      if (!options.noProgress) {
        const progressStream = createFileProgressStream(file.name, file.size)
...
function createFileProgressStream (filename, length) {
...
function getNodeByPath (path, folder) {
  path.forEach((folderName) => {
    ...
    const nodeIdFolder = folder.children.find(node => {
      const nodeId = node.nodeId || n.file.downloadId[1]
